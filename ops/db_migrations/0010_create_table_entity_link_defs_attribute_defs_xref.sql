...
(
    ...
    CONSTRAINT entity_def_fk       FOREIGN KEY(entity_link_def_id)    REFERENCES entity_link_defs(id),
    CONSTRAINT attribute_def_fk    FOREIGN KEY(attribute_def_id)      REFERENCES attribute_defs(id)
);

COMMENT ON COLUMN entity_link_defs_attribute_defs_xref.entity_link_def_id is 'The definition id of the entity link that has the attribute.';
