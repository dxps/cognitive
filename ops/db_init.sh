...
# Check if a custom user has been set, otherwise default to 'servicentral'.
DB_USER="${POSTGRES_USER:=servicentral}"
# Check if a custom password has been set, otherwise default to 'servicentral'.
DB_PASSWORD="${POSTGRES_PASSWORD:=servicentral}"
# Check if a custom password has been set, otherwise default to 'servicentral'.
DB_NAME="${POSTGRES_DB:=servicentral}"
...
then
  ...
  RUNNING_POSTGRES_CONTAINER=$(docker ps --filter 'name=servicentral' --format '{{.ID}}')
  ...
  docker run \
      ...
      -p "${DB_PORT}":5432 \
      -d \
      --name "servicentral_$(date '+%Y%m%d_%H%M%S')" \
      ${DB_IMAGE} -N 300
