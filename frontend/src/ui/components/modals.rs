use crate::{model::Action, ui::Route};
use dioxus::prelude::*;
use shlib::domain::model::{Id, ItemType};
...
pub struct AcknowledgeModalProps {
    ...
    pub links: Vec<(Id, String)>,

    #[props(default = ItemType::AttributeTemplate)]
    pub links_item_type: ItemType,

    pub action_handler: EventHandler,
}

fn get_link_target(links_item_type: &ItemType, id: Id) -> Route {
    match *links_item_type {
        ItemType::ObjectTemplate => Route::ObjectTemplateView { id: id },
        ItemType::Object => Route::ObjectView { id: id },
        ItemType::Link => Route::LinkView { id: id },
        _ => Route::AttributeTemplateView { id: id },
    }
}
                    ...
                    if !links.is_empty() {
                        ul {
                            for (id , name) in links {
                                li {
                                    Link { to: get_link_target(&links_item_type, id),
                                        {name}
                                    }
                                }
                            }
...
// TODO: Refactor to make action generic over different actions.
//       Currently only supports Delete action.
#[component]
