...
impl AttributeDefRepo {
    ...
    pub async fn get(&self, id: String) -> Option<AttributeDef> {
        ...
        sqlx::query_as::<_, AttributeDef>(
            "SELECT id, name, description, value_type, default_value, is_multivalued, is_composite, is_required, value_rules, tag_id FROM attribute_defs WHERE id = $1"
        )
            .bind(id)
    ...
    pub async fn list(&self, pagination_opts: Option<&PaginationOpts>) -> Vec<AttributeDef> {
        ...
        let query = format!(
            "SELECT id, name, description, value_type, default_value, is_multivalued, is_composite, is_required, value_rules, tag_id FROM attribute_defs ORDER BY id LIMIT {limit} OFFSET {offset}"
        );
...
impl FromRow<'_, PgRow> for AttributeDef {
    fn from_row(row: &PgRow) -> Result<Self, sqlx::Error> {
        Ok(Self {
            ...
            is_multivalued: row.get("is_multivalued"),
